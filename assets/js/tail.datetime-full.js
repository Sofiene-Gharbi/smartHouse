...
(function(root, factory) {
    ...
    datetime.prototype = {
        ...
        init: function() {
            ...
            // Init Select
            //fetch old data-value if exist
            if (this.e.getAttribute("data-value") != null || this.e.children[0].value != "") {
                this.select = parse(this.fetchDate(this.e.getAttribute("data-value")) || this.fetchDate(this.e.children[0].value, true));
            } else {
                this.select = false;
            }
            ...
            if (!this.select || this.select < this.con.dateStart || this.select > this.con.dateEnd) {
                this.select = null;
                this.e.children[0].value = ""; //TODO: to be replaced by the new values
                this.e.removeAttribute("data-value");
                if (this.view !== undefined) { this.view.date = new Date(this.view.date.setHours(0, 0, 0, 0)); }
            }

            // Init View
            if (this.view == undefined) {
                if (!this.con.timerActivate) {
                    this.view = {
                        type: this.con.viewDefault,
                        date: this.select || new Date(Date.now())
                    };
                    this.select = this.select || this.view.date;
                } else {
                    this.view = {
                        type: this.con.viewDefault,
                        date: this.select || new Date("", "", "", 0, 0, 0)
                    };
                    this.select = this.select || this.view.date;
                }
            }
            /*
            //select init time at 0:0:0 when we create a new datetime instance
            ...
            */
            ...
            let dataValue = this.e.getAttribute("data-value");
            this.dt = this.renderCalendar();
            if (dataValue != "") {
                this.e.setAttribute("data-value", dataValue);
            }
        ...
        prepare: function() {
            ...
            // Prepare Options
            this.con.dateStart = parse(this.con.dateStart, true) || -9999999999999;
            this.con.dateEnd = parse(this.con.dateEnd, true) || 9999999999999;
            ...
            if (event.type == "click") {
                ...
                switch (elem[a](d)) {
                    ...
                    case "submit":
                        ...
                        let calendarId = elem.parentElement.parentElement.parentElement.parentElement.parentElement.id,
                            totalTime = document.getElementById(`${calendarId}-hours`).value * 3600 * 1000 + document.getElementById(`${calendarId}-minutes`).value * 60 * 1000 + document.getElementById(`${calendarId}-seconds`).value * 1000,
                            total = totalTime + parseInt(elem[a]("data-date")),
                            D = new Date(total);
                        return this.selectDate(D);
        ...
        datetoTimerString: function(inDate, format) {
            var dateObject = {
                H: String("00" + inDate.getHours()).toString().slice(-2),
                i: String("00" + inDate.getMinutes()).toString().slice(-2),
                s: String("00" + inDate.getSeconds()).toString().slice(-2),
        ...
        renderCalendar: function() {
            var self = this,
                cls = ["tail-datetime-calendar", "calendar-close"],
            ...
            //close button original place
            // Render Close
            if (this.con.closeButton && !sta) {
                var close = create("BUTTON", "calendar-close");
        ...
        renderTimePicker: function(dt) {
            ...
            for (var key in { Hours: 0, Minutes: 0, Seconds: 0 }) {
                ...
                input.setAttribute("max", (key !== "Hours") ? "60" : (this.con.timerActivate) ? "100" : (this.con.time12h) ? "13" : "24");
                ...
                input.setAttribute("id", dt.id + "-" + key.toLowerCase());
            ...
            // Render View
            var div = create("DIV", "calendar-timepicker"),
                self = this;
        ...
        handleTime: function(input) {
            ...
            if (!this.con.timerActivate) {
                ...
                time[2].value = this.view.date.slice(-2);
                time[1].value = this.view.date.slice(3, 5);
                time[0].value = this.view.date.slice(0, 2);
            }
        },
        ...
        handleStep: function(input, action, prevent) {
            ...
            if (action === "up") {
                ...
                input.value = (value + step >= max) ? min : value + step;
            ...
            if (!this.con.timerActivate) {
                if (this.con.timeIncrement && inc !== null) {
                    ...
                    if (prev && prev.disabled === false) {
                        ...
                        let endDate = parse(this.con.dateEnd, true, true),
                            startDate = parse(this.con.dateStart, true, true),
                            viewDate = parse(this.view.date, true, true);
        ...
        viewDays: function() {
            var date = new Date(this.view.date.getTime()),
                ...
                tooltip = [0, 0],
                endDate = parse(this.con.dateEnd, true, true),
                startDate = parse(this.con.dateStart, true, true);
            ...
            while (r.length < 6) {
                ...
                // Calc Disabled
                if (this.con.dateBlacklist && ((time) < startDate || time > endDate)) {
                    disabled = [(time < startDate) ? startDate : Infinity, [0, 1, 2, 3, 4, 5, 6], true];
        ...
        selectDate: function(Y, M, D, H, I, S) {
            if (!this.con.timerActivate) {
                ...
                if (this.con.dateStart != -9999999999999) {
                    let startDate = new Date(this.con.dateStart),
                        startH = startDate.getHours(),
                        startMi = startDate.getMinutes(),
                        startS = startDate.getSeconds();

                    if (parse(this.con.dateStart, true, true) == parse(this.select.getTime(), true, true)) {
                        document.getElementById(`${this.dt.id}-hours`).setAttribute("min", startH);
                        document.getElementById(`${this.dt.id}-minutes`).setAttribute("min", startMi);
                        document.getElementById(`${this.dt.id}-seconds`).setAttribute("min", startS);
                        document.getElementById(`${this.dt.id}-hours`).setAttribute("value", startH < 10 ? "0" + startH : startH);
                        document.getElementById(`${this.dt.id}-minutes`).setAttribute("value", startMi < 10 ? "0" + startMi : startMi);
                        document.getElementById(`${this.dt.id}-seconds`).setAttribute("value", startS < 10 ? "0" + startS : startS);
                        if (document.getElementById(`${this.dt.id}-hours`).value < startH || document.getElementById(`${this.dt.id}-minutes`).value < startMi || document.getElementById(`${this.dt.id}-seconds`).value < startS) {
                            document.getElementById(`${this.dt.id}-hours`).value = startH;
                            document.getElementById(`${this.dt.id}-minutes`).value = startMi;
                            document.getElementById(`${this.dt.id}-seconds`).value = startS;
                        }
                        this.select.setHours(document.getElementById(`${this.dt.id}-hours`).value, document.getElementById(`${this.dt.id}-minutes`).value, document.getElementById(`${this.dt.id}-seconds`).value);
                    } else {
                        document.getElementById(`${this.dt.id}-hours`).setAttribute("min", "00");
                        document.getElementById(`${this.dt.id}-minutes`).setAttribute("min", "00");
                        document.getElementById(`${this.dt.id}-seconds`).setAttribute("min", "00");
                    }
                }
                ...
                this.e.children[0].value = this.convertDate(this.select, f.join(" "));
            } else {
                console.log(Y, M, D, H, I, S);
                if (Y != undefined && M == undefined && D == undefined && H == undefined && I == undefined && S == undefined) {
                ...
                H = H < 10 ? String("0" + H.toString()) : H;
                I = I < 10 ? String("0" + I.toString()) : I;
                S = S < 10 ? String("0" + S.toString()) : S;
                this.view.date = `${H}:${I}:${S}`;
                this.select = `${H}:${I}:${S}`;
                this.e.children[0].value = `${H}:${I}:${S}`;
            }
        ...
        reload: function() {
            let events = this.events,
                dataValue = this.e.getAttribute("data-value"),
                select = this.select,
                view = this.view;
            this.remove();
            ...
            this.select = select;
            this.e.setAttribute("data-value", dataValue);
            this.view.type = view.type;
            this.view.date = view.date;
            this.init();
            return this;
        },

        /*
         |  PUBLIC :: (G|S)ET OPTION
         |  @since  0.4.0 [0.4.0]
         */
